// This grammar against zu's conformance corpus.
//
// A grammar checked by its own test corpus is a grammar that agrees
// with its author. The corpus is a thousand statements the engine
// answers, so it is the one body of zuQL that was written without this
// grammar in mind, and every statement the engine accepts has to parse
// here with no error node in the tree. The statements the engine
// answers with a syntax error are the other half of the same question:
// a grammar that accepts everything is not a grammar.
//
// The statements come out of `cargo xtask grammar --queries` in zu,
// which reads the corpus with the engine's own reader. Nothing here
// reads a YAML file: a second reader of the corpus is a second thing to
// keep right, and the one in zu's `crates/zu-corpus` is the one the
// conformance runner uses.
//
// So this check needs a checkout of https://github.com/tamnd/zu. It
// looks at $ZU_ROOT, then at a sibling directory called zu, and it
// takes the statements ready-made from $ZU_GRAMMAR_QUERIES when that is
// set, which is what CI does when it has already run the engine's side.
...
const queries = process.env.ZU_GRAMMAR_QUERIES ?? join(here, "target", "grammar-queries");
...
if (!process.env.ZU_GRAMMAR_QUERIES) {
  const zu = process.env.ZU_ROOT ?? join(here, "..", "zu");
  if (!existsSync(join(zu, "Cargo.toml"))) {
    fail(
      `no checkout of zu at ${zu}. This check reads the conformance corpus ` +
        `with the engine's own reader, so point ZU_ROOT at a clone of ` +
        `https://github.com/tamnd/zu, or set ZU_GRAMMAR_QUERIES to a ` +
        `directory that cargo xtask grammar --queries has already written.`,
    );
  }
  execFileSync(
    "cargo",
    ["run", "-q", "-p", "xtask", "--", "grammar", "--queries", queries],
    { cwd: zu, stdio: ["ignore", "inherit", "inherit"] },
  );
}
...
const pathsFile = join(here, "target", "grammar-paths.txt");
